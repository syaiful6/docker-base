...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ] || [ "$1" = "unitd" ]; then
	if [ "$(id -u)" = '0' ]; then
		case "$1" in
			apache2*)
...
curl_put()
{
    RET=`/usr/bin/curl -s -w '%{http_code}' -X PUT --data-binary @$1 --unix-socket /usr/local/control.unit.sock http://localhost/$2`
    RET_BODY=${RET::-3}
...
if [ "$1" = "unitd" ]; then
    if /usr/bin/find "/usr/local/state" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            ...
            /usr/local/sbin/unitd --control unix:/usr/local/control.unit.sock

            while [ ! -S /usr/local/control.unit.sock ]; do echo "$0: Waiting for control socket to be created..."; /bin/sleep 0.1; done
            # even when the control socket exists, it does not mean unit has finished initialisation
            # this curl call will get a reply once unit is fully launched
            /usr/bin/curl -s -X GET --unix-socket /usr/local/control.unit.sock http://localhost/
            ...
            kill -TERM `/bin/cat /usr/local/unit.pid`

            while [ -S /usr/local/control.unit.sock ]; do echo "$0: Waiting for control socket to be removed..."; /bin/sleep 0.1; done
